...
from model_roi import VAEWithRouter
from nsd_dataloader import NSDDataset
...
def compute_activation(cfg, beta):
    return cfg.temp / beta

def train_one_epoch(model, dataloader, optimizer, cfg):
    model.train()
    epoch_loss, epoch_recon, epoch_kl = 0., 0., 0.
    for beta, img_clip in tqdm(dataloader, desc="Training"):
        img_clip, beta = img_clip.to(cfg.device), beta.to(cfg.device)
        ...
        total_loss, recon_loss, kl_loss, recon, mu, logvar, p_k = model.forward(
            ...
            router_l2_weight=cfg.l2_weight
        )
        optimizer.zero_grad()
        total_loss.backward()
        optimizer.step()
...
def validate(model, dataloader, cfg):
    ...
    for beta, img_clip in tqdm(dataloader, desc="Validation"):
        img_clip, beta = img_clip.to(cfg.device), beta.to(cfg.device)
        activation = compute_activation(cfg, beta)
        total_loss, recon_loss, kl_loss, recon, mu, logvar, p_k = model.forward(
            img_clip, activation,
...
def main(cfg, subregion_name):
    ...
    print(f"=== Training {cfg.roi} / {subregion_name} ===")
    mask = create_roi_mask(f'./ROI/floc-{cfg.roi}.nii.gz', roi_label=cfg.roi_label)
    train_dataset = NSDDataset('/cephfs/shared/nsd/session_npy', mask, session_list=list(range(1, 36)))
    test_dataset = NSDDataset('/cephfs/shared/nsd/session_npy', mask, session_list=list(range(36, 41)))
    ...
    model = VAEWithRouter(input_dim=1024, latent_dim=cfg.z_dim, hidden_dim=512, K=cfg.K).to(cfg.device)
    ...
    for epoch in range(cfg.epoch):
        ...
        scheduler.step()
        print(f"Epoch {epoch+1}/{cfg.epoch} | Train Loss: {train_loss:.4f} Train recon:{train_recon} Train kl:{train_kl} | Val Loss: {val_loss:.4f}")

        if val_loss < best_val_loss:
            best_val_loss = val_loss
            ckpt_file = f"checkpoints/{subregion_name}_K{cfg.K}_zdim{cfg.z_dim}_RW{cfg.recon_weight}_KLW{cfg.KLD_weight}_L2W{cfg.l2_weight}.pt"
...
if __name__ == "__main__":
    ...
    class Config:
        ...
        lr: float = 2e-4
        z_dim: int = 256
        recon_weight: float = 1.5
        KLD_weight: float = 1.0
        l2_weight: float = 60.0
        temp: float = 1.0
        K: int = 4
        roi: str = args.roi
